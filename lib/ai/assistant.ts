...
//
// Lite에는 그림 생성이 없다. 이미지 카드가 URL만 들고 있어 만들어 낸 바이트를 둘 곳이
// 없기 때문이다. 이미지 카드는 지우기 대상으로만 다룬다.
...
export const assistantSystemPrompt = [
    ...
    "",
    "그림(이미지):",
    "- KyuBoard Lite에서는 그림을 만들 수 없다. 이미지 카드는 사용자가 이미지 주소(URL)를 직접 넣어 만드는 것뿐이다.",
    "- 그림을 만들어 달라는 요청에는 만들 수 없다고 답하고, 다이어그램이면 mermaid를, 그림이면 이미지 주소를 직접 넣는 방법을 알려 준다.",
